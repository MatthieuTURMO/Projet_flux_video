...
architecture IMP of user_logic is
  ...
  --USER signal declarations added here, as needed for user logic
	-- multiplexer select
		signal mux_sel_d1 : std_logic;
		signal mux_sel_d2 : std_logic;
		signal mux_sel : std_logic;
		-- multiplexer module
		component my_xsvi_mux_ip is
		generic (
		C_XSVI_DWIDTH : INTEGER := 24;
		C_FAMILY : string := "spartan6"
		);
...
-- slv_reg0 is big endian, which means slv_reg0(31) is the least significant bit
			-- mux_sel is synchronized to video clock
			mux_sel_l : process (clk)
			begin
			if rising_edge(clk) then
			mux_sel_d1 <= slv_reg0(31);
			mux_sel_d2 <= mux_sel_d1;
			mux_sel <= mux_sel_d2;
			end if;
			end process;
			-- multiplexer module
			my_xsvi_mux_l : my_xsvi_mux_ip
			generic map (
